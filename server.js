...
const MAX_TRAIL_POINTS  = 2000;   // max positions per ship
const HISTORY_MAX_AGE   = 48 * 60 * 60 * 1000; // 48 hours in ms
const STATIONARY_SPEED  = 0.4;    // knots — below this = considered docked/anchored
const DEPART_SPEED      = 1.0;    // knots — above this = considered departed

// positionCache: latest static + position per MMSI
const positionCache = {};

// trailHistory: array of { lat, lon, ts } per MMSI — reset on port departure
const trailHistory  = {};

// lastSpeed: track previous speed to detect departure events
const lastSpeed     = {};
  ...
  aisWs.on('message', (data) => {
    try {
      ...
      if (positionCache[mmsi].static) {
        if (wasNew) {
          shipCount++;
          console.log(`[AIS] Ship online: ${positionCache[mmsi].static.Name || mmsi} (${shipCount} total)`);
        }
        broadcast({
          type:     'position',
          mmsi,
