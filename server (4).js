  ...
  aisWs.on('message', (data) => {
    try {
      ...
      if (msg.MessageType === 'ShipStaticData') {
        ...
        const dim = msg.Message.ShipStaticData.Dimension;
        const len = dim ? (dim.A || 0) + (dim.B || 0) : 0;

        // Only track passenger ships 100m+ (filters out ferries, small vessels)
        if (CRUISE_SHIP_TYPES.has(shipType) && (len === 0 || len >= 100)) {
          ...
          // Remove from cache if we previously stored it without size info
